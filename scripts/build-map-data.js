...
//     AIRTABLE_LAYER_MAP    Comma-separated  tableIdOrName:layer  pairs, e.g.
//                           tblRSY9kLo9sE8Ctb:active-sale,tblYhMRC7nJCThGGk:active-lease
//                           Valid layers: active-sale, active-lease, closed-sale,
...
function pairs(str) {
  // "key:value,key:value" -> Map. Splits on the FIRST colon only, so prefer
  // table IDs (tbl…, no colons). Table names work too if they contain no colon.
  const m = new Map();
...
const LAYER_MAP = pairs(process.env.AIRTABLE_LAYER_MAP);
...
async function run() {
  ...
  for (const t of considered) {
    const layer = LAYER_MAP.get(t.id) || LAYER_MAP.get(t.name);
    if (!layer) {
  ...
  for (const t of sources) {
    ...
    console.log(`\n[${t.name}] layer=${t.layer}; ${records.length} record(s).`);
